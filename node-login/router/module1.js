...
app.get('/about',(req,res)=>{
    ...
    app.get('/list',(req,res)=>{
        fs.readFile(__dirname + '/../data/member.json','utf8',(err,data)=>{
            if(!err){
                console.log(data);
                res.writeHead(200,{'content-type':'text/json;charset=utr-8'});
                res.end(data);
            }else{
                console.log(err);
            }
    ...
    app.get('/getMember/:userid',(req,res)=>{
        fs.readFile(__dirname + '/../data/member.json','utf8',(err,data)=>{//__dirname :현재폴더기준 (절대값)
            if(!err){
                const member = JSON.parse(data); //JSON형식으로 불러옴, 이렇게 안하면 글자형으로 갖고오기 때문
                res.json(member[req.params.userid]); //req,params.userid: 조건(요청한 파라미터는 userid)
            }else{
                console.log(err);
            }
        });
